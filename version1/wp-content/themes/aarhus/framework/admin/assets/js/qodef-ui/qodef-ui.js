(function($){
		...
		wp.media.customlibEditGallery1 = {
			frame: function () {
				...
				this._frame.on('update', function () {
					...
					var data = {
						action: 'aarhus_select_gallery_upload_get_images',
						ids: ids,
	...
	function qodefInitAjaxForm() {
		...
		$(document).delegate(".qodef_ajax_form", "submit", function (a) {
			var b = $(this),
				c = {
					action: "aarhus_select_save_options"
				};
		...
		if (holder.length) {
			...
			qodefImportBtn.on('click', function (e) {
				...
				if (confirm('Are you sure, you want to import Options now?')) {
					...
					var data = {
						action: 'aarhus_core_import_theme_options',
						content: importValue,
						nonce: importNonce
					};
		...
		if (holder.length) {
			...
			qodefImportBtn.on('click', function (e) {
				...
				if (confirm('Are you sure, you want to import Custom Sidebars now?')) {
					...
					var data = {
						action: 'aarhus_core_import_custom_sidebars',
						content: importValue,
						nonce: importNonce
					};
		...
		if (holder.length) {
			...
			qodefImportBtn.on('click', function (e) {
				...
				if (confirm('Are you sure, you want to import Widgets now?')) {
					...
					var data = {
						action: 'aarhus_core_import_widgets',
						content: importValue,
						nonce: importNonce
					};
			...
			qodefImportBtn.on('click', function (e) {
				...
				if (confirm(confirmMessage)) {
					...
					if (import_opt === 'content') {
						for (var i = 1; i < 10; i++) {
							var str = i < 10 ? 'aarhus_content_0' + i + '.xml' : 'aarhus_content_' + i + '.xml';
							
							jQuery.ajax({
								type: 'POST',
								url: ajaxurl,
								data: {
									action: 'aarhus_core_data_import',
									xml: str,
									example: import_expl,
						...
						$.ajax({
							...
							url: ajaxurl,
							data: {
								action: 'aarhus_core_widgets_import',
								example: import_expl
							},
							success: function (data, textStatus, XMLHttpRequest) {
						...
						jQuery.ajax({
							...
							url: ajaxurl,
							data: {
								action: 'aarhus_core_options_import',
								example: import_expl
							},
							success: function (data, textStatus, XMLHttpRequest) {
								$('.progress-value').html((100) + '%');
						...
						for (var i = 1; i < 10; i++) {
							var str = i < 10 ? str = 'aarhus_content_0' + i + '.xml' : 'aarhus_content_' + i + '.xml';
										...
										jQuery.ajax({
											...
											success: function (data, textStatus, XMLHttpRequest) {
												jQuery.ajax({
													...
													url: ajaxurl,
													data: {
														action: 'aarhus_core_other_import',
														example: import_expl
													},
													success: function (data, textStatus, XMLHttpRequest) {
														//alert(data);
