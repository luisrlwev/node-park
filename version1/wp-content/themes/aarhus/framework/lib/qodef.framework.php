...
/*
   Class: AarhusSelectClassFramework
...
class AarhusSelectClassFramework {
	...
	private function __construct() {
		$this->qodeOptions         = AarhusSelectClassOptions::get_instance();
		$this->qodeMetaBoxes       = AarhusSelectClassMetaBoxes::get_instance();
		$this->qodeTaxonomyOptions = AarhusSelectClassTaxonomyOptions::get_instance();
		$this->qodeUserOptions     = AarhusSelectClassUserOptions::get_instance();
		$this->qodeDashboardOptions = AarhusSelectClassDashboardOptions::get_instance();
	}
	...
	public function setSkin( AarhusSelectClassSkinAbstract $skinObject ) {
		$this->skin = $skinObject;
	}
}

/**
 * Class AarhusSelectClassSkinManager
 *
...
class AarhusSelectClassSkinManager {
	...
	/**
	 * @see AarhusSelectClassSkinManager::setSkin()
	 */
	...
	private function setSkin( $skinName = SELECT_PROFILE_SLUG ) {
		if ( $skinName !== '' ) {
			if ( file_exists( get_template_directory() . '/framework/admin/skins/' . $skinName . '/skin.php' ) ) {
				...
				$skinName = ucfirst( $skinName ) . esc_html__( 'Skin', 'aarhus' );
...
/**
 * Class AarhusSelectClassSkinAbstract
 *
 * Abstract class that each skin class must extend
 */
abstract class AarhusSelectClassSkinAbstract {
	/**
	 * @var string
	 */
	protected $skinName;
	...
	public function loadTemplatePart( $template, $params = array() ) {
		...
		if ( $template !== '' ) {
			include( aarhus_select_find_template_path( 'framework/admin/skins/' . $this->skinName . '/templates/' . $template . '.php' ) );
		}
	}
	...
	public function setShortcodeJSParams() { ?>
		<script>
			window.qodeSCIcon = '<?php echo aarhus_select_get_skin_uri() . '/assets/img/admin-logo-icon.png'; ?>';
			window.qodeSCLabel = '<?php echo esc_html( ucfirst( $this->skinName ) ); ?> <?php esc_html_e( 'Shortcodes', 'aarhus' ) ?>';
		</script>
	<?php }
...
/*
   Class: AarhusSelectClassOptions
...
class AarhusSelectClassOptions {
	...
	public function getOptionValue( $key ) {
		...
		if ( array_key_exists( $key, $aarhus_select_global_options ) ) {
			return $aarhus_select_global_options[ $key ];
...
/*
   Class: AarhusSelectClassAdminPage
   A class that initializes Select Admin Page
*/
class AarhusSelectClassAdminPage implements iAarhusSelectInterfaceLayoutNode {
	public $layout;
	private $factory;
	public $slug;
	public $title;
	...
	function __construct( $slug = "", $title_label = "", $icon = "" ) {
		$this->layout  = array();
		$this->factory = new AarhusSelectClassFieldFactory();
		$this->slug    = $slug;
	...
	public function renderChild( iAarhusSelectInterfaceRender $child ) {
		$child->render( $this->factory );
	}
}

/*
   Class: AarhusSelectClassMetaBoxes
...
/*
   Class: AarhusSelectClassMetaBox
   A class that initializes Select Meta Box
*/
class AarhusSelectClassMetaBox implements iAarhusSelectInterfaceLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	public $title;
	...
	public function setTitle( $label ) {
		...
		return $aarhus_select_global_Framework->getSkin()->getSkinLabel() . ' ' . $label;
	}
}

/*
   Class: AarhusSelectClassTaxonomyOptions
   A class that initializes AarhusSelectClass Taxonomy Options
*/
class AarhusSelectClassTaxonomyOptions {
	private static $instance;
...
/*
   Class: AarhusSelectClassTaxonomyOption
   A class that initializes AarhusSelectClass Taxonomy Option
*/
class AarhusSelectClassTaxonomyOption implements iAarhusSelectInterfaceLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	...
	function __construct( $scope = "" ) {
		$this->layout  = array();
		$this->factory = new AarhusSelectClassTaxonomyFieldFactory();
		$this->scope   = $scope;
	}
...
/*
   Class: AarhusSelectClassUserOptions
   A class that initializes AarhusSelectClass User Options
*/
class AarhusSelectClassUserOptions {
	private static $instance;
	public $userOptions;
...
/*
   Class: AarhusSelectClassUserOption
   A class that initializes AarhusSelectClass User Option
*/
class AarhusSelectClassUserOption implements iAarhusSelectInterfaceLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	...
	function __construct( $scope = "" ) {
		$this->layout  = array();
		$this->factory = new AarhusSelectClassUserFieldFactory();
		$this->scope   = $scope;
	}
...
/*
   Class: AarhusSelectClassDashboardOptions
   A class that initializes AarhusSelectClass Dashboard Options
*/
class AarhusSelectClassDashboardOptions {
	private static $instance;
...
/*
   Class: AarhusSelectClassDashboardOption
   A class that initializes AarhusSelectClass Dashboard Option
*/
class AarhusSelectClassDashboardOption implements iAarhusSelectInterfaceLayoutNode {
	public $layout;
	private $factory;
	
	function __construct() {
		$this->layout        = array();
		$this->factory       = new AarhusSelectClassDashboardFieldFactory();
	}
...
if ( ! function_exists( 'aarhus_select_init_framework_variable' ) ) {
	function aarhus_select_init_framework_variable() {
		global $aarhus_select_global_Framework;
		
		$aarhus_select_global_Framework = AarhusSelectClassFramework::get_instance();
		$qodeSkinManager                 = new AarhusSelectClassSkinManager();
		$aarhus_select_global_Framework->setSkin( $qodeSkinManager->getSkin() );
	}
	
	add_action( 'aarhus_select_action_before_options_map', 'aarhus_select_init_framework_variable' );
}
?>