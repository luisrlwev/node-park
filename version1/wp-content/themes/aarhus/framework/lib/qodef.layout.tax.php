...
/*
   Class: AarhusSelectClassTaxonomyField
   A class that initializes AarhusSelectClass Taxonomy Field
*/

class AarhusSelectClassTaxonomyField implements iAarhusSelectInterfaceRender {
	private $type;
	private $name;
	private $label;
	private $description;
	...
	function __construct( $type, $name, $label = "", $description = "", $options = array(), $args = array(), $hidden_property = "", $hidden_values = array() ) {
		...
		add_filter( 'aarhus_select_filter_taxonomy_fields', array( $this, 'addFieldForEditSave' ) );
	}
...
abstract class AarhusSelectClassTaxonomyFieldType {
	abstract public function render( $name, $label = "", $description = "", $options = array(), $args = array() );
}

class AarhusSelectClassTaxonomyFieldText extends AarhusSelectClassTaxonomyFieldType {
...
class AarhusSelectClassTaxonomyFieldTextArea extends AarhusSelectClassTaxonomyFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $hidden = false ) {
		if ( ! isset( $_GET['tag_ID'] ) ) { ?>
			<div class="form-field">
				<label for="<?php echo esc_attr( $name ); ?>"><?php echo esc_html( $label ); ?></label>
...
class AarhusSelectClassTaxonomyFieldImage extends AarhusSelectClassTaxonomyFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $hidden = false) {
		if ( ! isset( $_GET['tag_ID'] ) ) { ?>
			<div class="form-field">
				<label for="<?php echo esc_attr( $name ); ?>"><?php echo esc_html( $label ); ?></label>
				...
				<p>
					<input type="button" class="button button-secondary qodef-tax-media-add" name="qodef-tax-media-add" value="<?php esc_attr_e( 'Add Image', 'aarhus' ); ?>"/>
					<input type="button" class="button button-secondary qodef-tax-media-remove" name="qodef-tax-media-remove" value="<?php esc_attr_e( 'Remove Image', 'aarhus' ); ?>"/>
				</p>
			...
			<tr class="form-field <?php echo esc_attr( $field_class ); ?>">
				...
				<td>
					...
					<p>
						<input type="button" class="button button-secondary qodef-tax-media-add" name="qodef-tax-media-add" value="<?php esc_attr_e( 'Add Image', 'aarhus' ); ?>"/>
						<input data-termid="<?php echo esc_attr( $_GET['tag_ID'] ); ?>" data-taxonomy="<?php echo esc_attr( $taxonomy ); ?>" type="button" class="button button-secondary qodef-tax-media-remove" name="qodef-tax-media-remove" value="<?php esc_attr_e( 'Remove Image', 'aarhus' ); ?>"/>
					</p>
				</td>
			</tr>
			<?php
...
class AarhusSelectClassTaxonomyFieldSelect extends AarhusSelectClassTaxonomyFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $hidden = false ) {
...
class AarhusSelectClassTaxonomyFieldSelectBlank extends AarhusSelectClassTaxonomyFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $hidden = false ) {
...
class AarhusSelectClassTaxonomyFieldCheckBoxGroup extends AarhusSelectClassTaxonomyFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $hidden = false ) {
...
class AarhusSelectClassTaxonomyFieldIcon extends AarhusSelectClassTaxonomyFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $hidden = false ) {
		...
		if ( ! isset( $_GET['tag_ID'] ) ) { ?>
			...
			<?php foreach ( $icons_collections as $icons_collection ) {
				...
				<div class="form-field qode-icon-collection-holder qodef-hide" data-icon-collection="<?php echo esc_attr( $icons_collection ); ?>">
					<label for="<?php echo esc_attr( $name ) . '_icon'; ?>"><?php esc_html_e( 'Icon', 'aarhus' ); ?></label>
			...
			<?php foreach ( $icons_collections as $icons_collection ) {
				...
				<tr class="form-field qode-icon-collection-holder <?php echo esc_attr( $field_class ); ?>" data-icon-collection="<?php echo esc_attr( $icons_collection ); ?>">
					<th scope="row"><?php esc_html_e( 'Icon', 'aarhus' ); ?></th>
					<td>
						<select name="<?php echo esc_attr( $name . '_' . $icons_param ) ?>" id="<?php echo esc_attr( $name . '_' . $icons_param ) ?>">
							<?php
							$icons       = aarhus_select_icon_collections()->getIconCollection( $icons_collection );
...
class AarhusSelectClassTaxonomyFieldColor extends AarhusSelectClassTaxonomyFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $hidden = false ) {
		if ( ! isset( $_GET['tag_ID'] ) ) { ?>
			<div class="form-field">
				<label for="<?php echo esc_attr( $name ); ?>"><?php echo esc_html( $label ); ?></label>
...
class AarhusSelectClassTaxonomyFieldFactory {
	public function render( $field_type, $name, $label = "", $description = "", $options = array(), $args = array(), $hidden = false ) {
		switch ( strtolower( $field_type ) ) {
			case 'text':
				$field = new AarhusSelectClassTaxonomyFieldText();
				$field->render( $name, $label, $description, $options, $args, $hidden );
				break;
			
			case 'textarea':
				$field = new AarhusSelectClassTaxonomyFieldTextArea();
				$field->render( $name, $label, $description, $options, $args, $hidden );
				break;
			
			case 'image':
				$field = new AarhusSelectClassTaxonomyFieldImage();
				$field->render( $name, $label, $description, $options, $args, $hidden );
				break;
			
			case 'select':
				$field = new AarhusSelectClassTaxonomyFieldSelect();
				$field->render( $name, $label, $description, $options, $args, $hidden );
				break;
			
			case 'selectblank':
				$field = new AarhusSelectClassTaxonomyFieldSelectBlank();
				$field->render( $name, $label, $description, $options, $args, $hidden );
				break;
			
			case 'checkboxgroup':
				$field = new AarhusSelectClassTaxonomyFieldCheckBoxGroup();
				$field->render( $name, $label, $description, $options, $args, $hidden );
				break;
			
			case 'icon':
				$field = new AarhusSelectClassTaxonomyFieldIcon();
				$field->render( $name, $label, $description, $options, $args, $hidden );
				break;
			
			case 'color':
				$field = new AarhusSelectClassTaxonomyFieldColor();
				$field->render( $name, $label, $description, $options, $args, $hidden );
				break;
