...
/*
   Class: AarhusSelectClassDashboardForm
   A class that initializes Select Dashboard Container
*/

class AarhusSelectClassDashboardForm implements iAarhusSelectInterfaceLayoutNode, iAarhusSelectInterfaceRender {
	public $children;
	public $name;
	public $form_id;
	public $form_method;
...
/*
   Class: AarhusSelectClassDashboardGroup
   A class that initializes AarhusSelectClass Group Field
*/
class AarhusSelectClassDashboardGroup implements iAarhusSelectInterfaceLayoutNode, iAarhusSelectInterfaceRender {
	public $children;
	public $name;
	public $title;
	public $description;
...
/*
   Class: AarhusSelectClassDashboardTitle
   A class that initializes Dashboard Title
*/
class AarhusSelectClassDashboardTitle implements iAarhusSelectInterfaceRender {
	private $name;
	private $title;
	private $args = array();
...
/*
   Class: AarhusSelectClassDashboardField
   A class that initializes AarhusSelectClass Front Field
*/
class AarhusSelectClassDashboardField implements iAarhusSelectInterfaceRender {
	private $type;
	private $name;
	private $label;
	private $description;
...
abstract class AarhusSelectClassDashboardFieldType {
	abstract public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = "" );
}

class AarhusSelectClassDashboardFieldText extends AarhusSelectClassDashboardFieldType {
...
class AarhusSelectClassDashboardFieldTextArea extends AarhusSelectClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$col_width = 12;
...
class AarhusSelectClassDashboardFieldImage extends AarhusSelectClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$class = '';
			...
			<div class="qodef-dashboard-gallery-holder">
				<ul class="qodef-dashboard-gallery-images-holder">
					<?php if ( isset( $value_html ) ) {
						echo aarhus_select_get_module_part($value_html);
					} ?>
				</ul>
				<div class="qodef-dashboard-gallery-uploader">
					<?php
					if ( aarhus_membership_theme_installed() ) {
						echo aarhus_select_get_button_html( array(
							'text'         => esc_html__( 'Upload', 'aarhus' ),
							'custom_class' => 'qodef-dashboard-gallery-upload'
						) );
					} else {
						echo '<a itemprop="url" href="#" class="qodef-btn qodef-btn-medium qodef-btn-solid qodef-dashboard-gallery-upload"><span class="qodef-btn-text">' . esc_html__( 'Upload', 'aarhus' ) . '</span></a>';
					} ?>
					...
					<?php if ( $value !== '' && $value !== false ) { ?>
						<button class="qodef-btn qodef-btn-solid qodef-dashboard-remove-image"><?php esc_html_e( 'Remove Media', 'aarhus' ); ?></button>
					<?php } ?>
				</div>
			</div>
		</div>
...
class AarhusSelectClassDashboardFieldGallery extends AarhusSelectClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$class = '';
...
class AarhusSelectClassDashboardFieldSelect extends AarhusSelectClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$class = '';
...
class AarhusSelectClassDashboardFieldDate extends AarhusSelectClassFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		if ( ! empty( $repeat ) && array_key_exists( 'name', $repeat ) && array_key_exists( 'index', $repeat ) ) {
			$id   = $name . '-' . $repeat['index'];
			$name = $repeat['name'] . '[' . $repeat['index'] . '][' . $name . ']';
...
class AarhusSelectClassDashboardFieldIcon extends AarhusSelectClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$options           = aarhus_select_icon_collections()->getIconCollectionsEmpty();
		$icons_collections = aarhus_select_icon_collections()->getIconCollectionsKeys();
				...
				<div class="qodef-dashboard-item">
					...
					<div class="qodef-dashboard-icon-holder">
						...
						<?php foreach ( $icons_collections as $icons_collection ) {
							$icons_param = aarhus_select_icon_collections()->getIconCollectionParamNameByKey( $icons_collection );
							...
							<div class="qodef-icon-collection-holder <?php echo esc_attr( $field_class ); ?>" data-icon-collection="<?php echo esc_attr( $icons_collection ); ?>">
								<select name="<?php echo esc_attr( $name . '[' . $icons_param . ']' ); ?>" id="<?php echo esc_attr( $name . '[' . $icons_param . ']' ); ?>">
									<?php
									$icons       = aarhus_select_icon_collections()->getIconCollection( $icons_collection );
...
class AarhusSelectClassDashboardFieldColor extends AarhusSelectClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		if ( ! empty( $repeat ) && array_key_exists( 'name', $repeat ) && array_key_exists( 'index', $repeat ) ) {
			$id   = $name . '-' . $repeat['index'];
			$name = $repeat['name'] . '[' . $repeat['index'] . '][' . $name . ']';
...
class AarhusSelectClassDashboardFieldCheckBoxGroup extends AarhusSelectClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$col_width = 12;
...
class AarhusSelectClassDashboardFieldAddress extends AarhusSelectClassFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$col_width = 12;
		if ( isset( $args["col_width"] ) ) {
			$col_width = $args["col_width"];
			...
			<div class="qodef-dashboard-field-row">
				<div class="qodef-dashboard-item col-lg-<?php echo esc_attr( $col_width ); ?>">
					<div class="qodef-dashboard-input-holder">
						...
						<a class="qodef-reset-marker qodef-hide-field" href="#"><?php esc_html_e( 'Reset Marker', 'aarhus' ); ?></a>
...
class AarhusSelectClassDashboardRepeater implements iAarhusSelectInterfaceRender {
	private $label;
	private $description;
	private $name;
	private $fields;
	...
	function __construct( $fields, $name, $label = '', $description = '', $button_text = '', $table_layout = false, $value = array() ) {
		...
		$this->name         = $name;
		$this->num_of_rows  = 1;
		$this->button_text  = ! empty( $button_text ) ? $button_text : esc_html__( 'Add New Item', 'aarhus' );
			...
			<?php if ( $this->table_layout ) { ?>
				<div class="qodef-dashboard-repeater-table-heading">
					<div class="qodef-dashboard-repeater-fields-holder">
						<div class="qodef-dashboard-repeater-table-cell qodef-dashboard-repeater-sort"><?php esc_html_e( 'Order', 'aarhus' ) ?></div>
						...
						<div class="qodef-dashboard-repeater-table-cell qodef-dashboard-repeater-remove"><?php esc_html_e( 'Remove', 'aarhus' ) ?></div>
					</div>
				</div>
			<?php } ?>
...
class AarhusSelectClassDashboardFieldFactory {
	public function render( $field_type, $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		switch ( strtolower( $field_type ) ) {
			case 'text':
				$field = new AarhusSelectClassDashboardFieldText();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'textarea':
				$field = new AarhusSelectClassDashboardFieldTextArea();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'date':
				$field = new AarhusSelectClassDashboardFieldDate();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'image':
				$field = new AarhusSelectClassDashboardFieldImage();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'gallery':
				$field = new AarhusSelectClassDashboardFieldGallery();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'select':
				$field = new AarhusSelectClassDashboardFieldSelect();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'icon':
				$field = new AarhusSelectClassDashboardFieldIcon();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'color':
				$field = new AarhusSelectClassDashboardFieldColor();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'checkboxgroup':
				$field = new AarhusSelectClassDashboardFieldCheckBoxGroup();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'address':
				$field = new AarhusSelectClassDashboardFieldAddress();
