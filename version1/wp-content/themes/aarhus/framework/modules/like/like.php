...
class AarhusSelectClassLike {
	private static $instance;
	
	private function __construct() {
		add_action( 'wp_ajax_aarhus_select_like', array( $this, 'ajax' ) );
		add_action( 'wp_ajax_nopriv_aarhus_select_like', array( $this, 'ajax' ) );
	}
	...
	public function add_like() {
		...
		$title       = esc_html__( 'Like this', 'aarhus' );
		
		if ( isset( $_COOKIE[ 'qodef-like_' . $post->ID ] ) ) {
			$class = 'qodef-like liked';
			$title = esc_html__( 'You already like this!', 'aarhus' );
		}
...
if ( ! function_exists( 'aarhus_select_create_like' ) ) {
	function aarhus_select_create_like() {
		AarhusSelectClassLike::get_instance();
	}
	
	add_action( 'after_setup_theme', 'aarhus_select_create_like' );
}