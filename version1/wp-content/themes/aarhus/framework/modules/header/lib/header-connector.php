...
namespace AarhusSelectNamespace\Modules\Header\Lib;

/**
 * Class AarhusSelectClassHeaderConnector
 *
...
class AarhusSelectClassHeaderConnector {
	...
	public function connect( $config = array() ) {
		do_action( 'aarhus_select_action_pre_header_connect' );
		...
		if ( ! empty( $config['affect_content'] ) ) {
			add_filter( 'aarhus_select_filter_content_elem_style_attr', array( $this, 'contentMarginFilter' ) );
		}
		
		if ( ! empty( $config['affect_title'] ) ) {
			add_filter( 'aarhus_select_filter_title_content_padding', array( $this, 'titlePaddingFilter' ) );
		}
		
		do_action( 'aarhus_select_action_after_header_connect' );
	}
	 ...
	 *
	 * Hooks to aarhus_select_filter_title_content_padding filter
	 *
	 * @return int
	 */
