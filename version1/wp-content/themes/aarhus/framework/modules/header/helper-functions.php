...
if ( ! function_exists( 'aarhus_select_header_skin_class' ) ) {
	/**
	 * Function that adds header style class to body tag
	 */
	function aarhus_select_header_skin_class( $classes ) {
		$header_style     = aarhus_select_get_meta_field_intersect( 'header_style', aarhus_select_get_page_id() );
		$header_style_404 = aarhus_select_options()->getOptionValue( '404_header_style' );
	...
	add_filter( 'body_class', 'aarhus_select_header_skin_class' );
}

if ( ! function_exists( 'aarhus_select_sticky_header_behaviour_class' ) ) {
	/**
	 * Function that adds header behavior class to body tag
	 */
	function aarhus_select_sticky_header_behaviour_class( $classes ) {
		$header_behavior = aarhus_select_get_meta_field_intersect( 'header_behaviour', aarhus_select_get_page_id() );
	...
	add_filter( 'body_class', 'aarhus_select_sticky_header_behaviour_class' );
}

if ( ! function_exists( 'aarhus_select_menu_dropdown_appearance' ) ) {
	/**
	 ...
	 */
	function aarhus_select_menu_dropdown_appearance( $classes ) {
		$dropdown_menu_appearance = aarhus_select_options()->getOptionValue( 'menu_dropdown_appearance' );
	...
	add_filter( 'body_class', 'aarhus_select_menu_dropdown_appearance' );
}

if ( ! function_exists( 'aarhus_select_header_class' ) ) {
	/**
	 ...
	 */
	function aarhus_select_header_class( $classes ) {
		$id = aarhus_select_get_page_id();
		
		$header_type = aarhus_select_get_meta_field_intersect( 'header_type', $id );
		
		$classes[] = 'qodef-' . $header_type;
		
		$disable_menu_area_shadow = aarhus_select_get_meta_field_intersect( 'menu_area_shadow', $id ) == 'no';
		if ( $disable_menu_area_shadow ) {
			$classes[] = 'qodef-menu-area-shadow-disable';
		}
		
		$disable_menu_area_grid_shadow = aarhus_select_get_meta_field_intersect( 'menu_area_in_grid_shadow', $id ) == 'no';
		if ( $disable_menu_area_grid_shadow ) {
			$classes[] = 'qodef-menu-area-in-grid-shadow-disable';
		}
		
		$disable_menu_area_border = aarhus_select_get_meta_field_intersect( 'menu_area_border', $id ) == 'no';
		if ( $disable_menu_area_border ) {
			$classes[] = 'qodef-menu-area-border-disable';
		}
		
		$disable_menu_area_grid_border = aarhus_select_get_meta_field_intersect( 'menu_area_in_grid_border', $id ) == 'no';
		if ( $disable_menu_area_grid_border ) {
			$classes[] = 'qodef-menu-area-in-grid-border-disable';
		}
		
		if ( aarhus_select_get_meta_field_intersect( 'menu_area_in_grid', $id ) == 'yes' &&
		     aarhus_select_get_meta_field_intersect( 'menu_area_grid_background_color', $id ) !== '' &&
		     aarhus_select_get_meta_field_intersect( 'menu_area_grid_background_transparency', $id ) !== '0'
		) {
			$classes[] = 'qodef-header-menu-area-in-grid-padding';
		}
		
		$disable_logo_area_border = aarhus_select_get_meta_field_intersect( 'logo_area_border', $id ) == 'no';
		if ( $disable_logo_area_border ) {
			$classes[] = 'qodef-logo-area-border-disable';
		}
		
		$disable_logo_area_grid_border = aarhus_select_get_meta_field_intersect( 'logo_area_in_grid_border', $id ) == 'no';
		if ( $disable_logo_area_grid_border ) {
			$classes[] = 'qodef-logo-area-in-grid-border-disable';
		}
		
		if ( aarhus_select_get_meta_field_intersect( 'logo_area_in_grid', $id ) == 'yes' &&
		     aarhus_select_get_meta_field_intersect( 'logo_area_grid_background_color', $id ) !== '' &&
		     aarhus_select_get_meta_field_intersect( 'logo_area_grid_background_transparency', $id ) !== '0'
		) {
			$classes[] = 'qodef-header-logo-area-in-grid-padding';
		}
		
		$disable_shadow_vertical = aarhus_select_get_meta_field_intersect( 'vertical_header_shadow', $id ) == 'no';
		if ( $disable_shadow_vertical ) {
			$classes[] = 'qodef-header-vertical-shadow-disable';
		}
		
		$disable_border_vertical = aarhus_select_get_meta_field_intersect( 'vertical_header_border', $id ) == 'no';
	...
	add_filter( 'body_class', 'aarhus_select_header_class' );
}

if ( ! function_exists( 'aarhus_select_header_area_style' ) ) {
	/**
	 * Function that return styles for header area
	 */
	function aarhus_select_header_area_style( $style ) {
		...
		$menu_area_background_color_rgba = aarhus_select_rgba_color( $menu_area_background_color, $menu_area_background_transparency );
		...
		if ( $menu_area_height !== '' ) {
			$menu_area_style['height'] = aarhus_select_filter_px( $menu_area_height ) . 'px !important';
		}
		...
		$container_side_padding_left  = get_post_meta( $page_id, 'qodef_menu_area_side_padding_left_meta', true );
		$container_side_padding_right = get_post_meta( $page_id, 'qodef_menu_area_side_padding_right_meta', true );
		...
		if ( $container_side_padding_left !== '' ) {
			...
			$current_style .= aarhus_select_dynamic_css( $menu_container_selector, $menu_container_styles );
		}
		...
		$menu_area_grid_background_color_rgba = aarhus_select_rgba_color( $menu_area_grid_background_color, $menu_area_grid_background_transparency );
		
		if ( ! empty( $menu_area_grid_background_color_rgba ) ) {
			$menu_area_grid_style['background-color'] = $menu_area_grid_background_color_rgba;
		}
		
		$current_style .= aarhus_select_dynamic_css( $menu_area_selector, $menu_area_style );
		$current_style .= aarhus_select_dynamic_css( $menu_area_grid_selector, $menu_area_grid_style );
		...
		if ( $dropdown_top_position !== '' ) {
			$dropdown_styles['top'] = aarhus_select_filter_suffix( $dropdown_top_position, '%' ) . '%';
		}
		
		$dropdown_selector = array( $class_prefix . ' .qodef-page-header .qodef-drop-down .second' );
		
		$current_style .= aarhus_select_dynamic_css( $dropdown_selector, $dropdown_styles );
		...
		$logo_area_background_color_rgba = aarhus_select_rgba_color( $logo_area_background_color, $logo_area_background_transparency );
		...
		if ( $logo_area_height !== '' ) {
			$logo_area_style['height'] = aarhus_select_filter_px( $logo_area_height ) . 'px !important';
		}
		...
		$logo_area_grid_background_color_rgba = aarhus_select_rgba_color( $logo_area_grid_background_color, $logo_area_grid_background_transparency );
		...
		if ( ! empty( $logo_area_style ) ) {
			$current_style .= aarhus_select_dynamic_css( $logo_area_selector, $logo_area_style );
		}
		
		if ( ! empty( $logo_area_grid_style ) ) {
			$current_style .= aarhus_select_dynamic_css( $logo_area_grid_selector, $logo_area_grid_style );
		}
		
		$current_style = apply_filters( 'aarhus_select_filter_add_header_page_custom_style', $current_style, $class_prefix, $page_id ) . $style;
		
		return $current_style;
	}
	
	add_filter( 'aarhus_select_filter_add_page_custom_style', 'aarhus_select_header_area_style' );
}