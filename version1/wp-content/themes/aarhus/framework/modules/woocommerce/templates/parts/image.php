...
if ( $product->is_on_sale() ) { ?>
	<span class="qodef-<?php echo esc_attr( $class_name ); ?>-onsale"><?php echo aarhus_select_get_woocommerce_sale( $product ); ?></span>
<?php } ?>

<?php if ( ! $product->is_in_stock() ) { ?>
	<span class="qodef-<?php echo esc_attr( $class_name ); ?>-out-of-stock"><?php esc_html_e( 'Sold', 'aarhus' ); ?></span>
<?php }

$new = get_post_meta( get_the_ID(), 'qodef_show_new_sign_woo_meta', true );

if ( $new === 'yes' ) { ?>
	<span class="qodef-<?php echo esc_attr( $class_name ); ?>-new-product"><?php esc_html_e( 'New', 'aarhus' ); ?></span>
<?php }
...
if ( $image_size === 'full' ) {
	...
	$product_image_size = 'aarhus_select_image_square';
} else if ( $image_size === 'landscape' ) {
	$product_image_size = 'aarhus_select_image_landscape';
} else if ( $image_size === 'portrait' ) {
	$product_image_size = 'aarhus_select_image_portrait';
...
if ( ! empty( $fixed_image_size_meta ) && isset( $type ) && $type === 'masonry' ) {
	if ( $fixed_image_size_meta === 'small' ) {
		...
		$product_image_size = 'aarhus_select_image_huge';
	}
}

if ( has_post_thumbnail() ) {
	the_post_thumbnail( apply_filters( 'aarhus_select_filter_product_list_image_size', $product_image_size ) );
}