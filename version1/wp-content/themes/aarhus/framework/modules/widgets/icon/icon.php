...
class AarhusSelectClassIconWidget extends AarhusSelectClassWidget {
	public function __construct() {
		parent::__construct(
			'qodef_icon_widget',
			esc_html__( 'Aarhus Icon Widget', 'aarhus' ),
			array( 'description' => esc_html__( 'Add icons to widget areas', 'aarhus' ) )
		);
	...
	protected function setParams() {
		$this->params = array_merge(
			aarhus_select_icon_collections()->getIconWidgetParamsArray(),
			array(
				array(
					'type'  => 'textfield',
					'name'  => 'icon_text',
					'title' => esc_html__( 'Icon Text', 'aarhus' )
				),
				array(
					'type'  => 'textfield',
					'name'  => 'link',
				...
				array(
					'type'    => 'dropdown',
					'name'    => 'target',
					'title'   => esc_html__( 'Target', 'aarhus' ),
					'options' => aarhus_select_get_link_target_array()
				),
				array(
					'type'  => 'textfield',
					'name'  => 'icon_size',
					'title' => esc_html__( 'Icon Size (px)', 'aarhus' )
				),
				array(
					'type'  => 'colorpicker',
					'name'  => 'icon_color',
					'title' => esc_html__( 'Icon Color', 'aarhus' )
				),
				array(
					'type'  => 'colorpicker',
					'name'  => 'icon_hover_color',
					'title' => esc_html__( 'Icon Hover Color', 'aarhus' )
				),
				array(
					'type'  => 'textfield',
					'name'  => 'text_size',
					'title' => esc_html__( 'Text Size (px)', 'aarhus' )
				),
				array(
					'type'  => 'colorpicker',
					'name'  => 'text_color',
					'title' => esc_html__( 'Text Color', 'aarhus' )
				),
				array(
					'type'        => 'textfield',
					'name'        => 'margin',
	...
	public function widget( $args, $instance ) {
		...
		$data   = array();
		$data[] = ! empty( $instance['icon_hover_color'] ) ? aarhus_select_get_inline_attr( $instance['icon_hover_color'], 'data-hover-color' ) : '';
        $data[] = ! empty( $instance['icon_color'] ) ? aarhus_select_get_inline_attr( $instance['icon_color'], 'data-color' ) : '';
		$data   = implode( ' ', $data );
		...
		if ( ! empty( $instance['icon_size'] ) ) {
			$icon_styles[] = 'font-size: ' . aarhus_select_filter_px( $instance['icon_size'] ) . 'px';
		}
		
		$text_styles = array();
		if ( ! empty( $instance['text_size'] ) ) {
			$text_styles[] = 'font-size: ' . aarhus_select_filter_px( $instance['text_size'] ) . 'px';
		}
		...
		if ( ! empty( $instance['icon_pack'] ) ) {
			...
			if ( ! empty( $icon_class ) ) {
				...
				$icon_holder_html = '<span class="qodef-icon-element ' . esc_attr( $icon_class ) . '" ' . aarhus_select_get_inline_style( $icon_styles ) . '></span>';
			}
		}
		...
		if ( ! empty( $instance['icon_text'] ) ) {
			$icon_text_html = '<span class="qodef-icon-text ' . esc_attr( $icon_text_class ) . '" ' . aarhus_select_get_inline_style( $text_styles ) . '>' . esc_html( $instance['icon_text'] ) . '</span>';
		}
		?>
		
		<a <?php aarhus_select_class_attribute( $holder_classes ); ?> <?php echo wp_kses_post( $data ); ?> href="<?php echo esc_url( $link ); ?>" target="<?php echo esc_attr( $target ); ?>" <?php echo aarhus_select_get_inline_style( $holder_styles ); ?>>
