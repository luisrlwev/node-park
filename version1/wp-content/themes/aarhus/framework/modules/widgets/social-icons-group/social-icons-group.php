...
class AarhusSelectClassClassIconsGroupWidget extends AarhusSelectClassWidget {
	public function __construct() {
		parent::__construct(
			'qodef_social_icons_group_widget',
			esc_html__( 'Aarhus Social Icons Group Widget', 'aarhus' ),
			array( 'description' => esc_html__( 'Use this widget to add a group of up to 6 social icons to a widget area.', 'aarhus' ) )
		);
	...
	protected function setParams() {
		$this->params = array_merge(
			...
			aarhus_select_icon_collections()->getSocialIconWidgetMultipleParamsArray(6),
			array(
				array(
					'type'    => 'dropdown',
					'name'    => 'layout',
					'title'   => esc_html__( 'Icons Layout', 'aarhus' ),
					'options' => array(
						''             => esc_html__( 'Default', 'aarhus' ),
						'square-icons' => esc_html__( 'Square', 'aarhus' ),
					)
				),
				array(
					'type'        => 'dropdown',
					'name'        => 'skin',
					'title'       => esc_html__( 'Square Icons Skin', 'aarhus' ),
					'description' => esc_html__( 'This applies to the square layout', 'aarhus' ),
					'options'     => array(
						''           => esc_html__( 'Dark Skin', 'aarhus' ),
						'light-skin' => esc_html__( 'Light Skin', 'aarhus' ),
					)
				),
				array(
					'type'    => 'dropdown',
					'name'    => 'alignment',
					'title'   => esc_html__( 'Text Alignment', 'aarhus' ),
					'options' => array(
				...
				array(
					'type'  => 'textfield',
					'name'  => 'icon_size',
					'title' => esc_html__( 'Icons Size (px)', 'aarhus' )
				),
				array(
					'type'  => 'colorpicker',
					'name'  => 'color',
	...
	public function widget( $args, $instance ) {
		...
		for ( $n = 1; $n <= 6; $n++ ) {
			...
			<?php if ( ! empty( $icon_holder_html ) ) { ?>
				<a class="qodef-social-icon-widget-holder qodef-icon-has-hover" <?php echo aarhus_select_get_inline_attr( $hover_color, 'data-hover-color' ); ?> <?php aarhus_select_inline_style( $icon_styles ) ?>
