<?php
$author_info_box       = esc_attr( aarhus_select_options()->getOptionValue( 'blog_author_info' ) );
$author_info_email     = esc_attr( aarhus_select_options()->getOptionValue( 'blog_author_info_email' ) );
$author_id             = esc_attr( get_the_author_meta( 'ID' ) );
$social_networks       = aarhus_select_core_plugin_installed() ? aarhus_select_get_user_custom_fields() : false;
$display_author_social = aarhus_select_options()->getOptionValue( 'blog_single_author_social' ) === 'no' ? false : true;
?>
			...
			<div class="qodef-author-description-content">
				<div class="qodef-author-description-image">
					<a itemprop="url" href="<?php echo esc_url( get_author_posts_url( $author_id ) ); ?>" title="<?php the_title_attribute(); ?>">
						<?php echo aarhus_select_kses_img( get_avatar( get_the_author_meta( 'ID' ), 260 ) ); ?>
					</a>
				</div>
				<div class="qodef-author-description-text-holder">
                    <h3 class="qodef-author-name vcard author">
                        <span class="qodef-author-name-label"><?php esc_html_e('Written by:', 'aarhus'); ?></span>
                        <a itemprop="url" href="<?php echo esc_url( get_author_posts_url( $author_id ) ); ?>" title="<?php the_title_attribute(); ?>">
							<span class="fn">
								<?php
							...
							<div class="qodef-author-social-icons clearfix">
								<?php foreach ( $social_networks as $network ) { ?>
									<a itemprop="url" href="<?php echo esc_url( $network['link'] ) ?>" target="_blank">
										<?php echo aarhus_select_icon_collections()->renderIcon( $network['class'], 'font_elegant' ); ?>
									</a>
								<?php } ?>
							</div>
						<?php } ?>
