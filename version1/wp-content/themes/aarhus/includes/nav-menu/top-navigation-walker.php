...
if (!class_exists('AarhusSelectClassTopNavigationWalker')) {
	class AarhusSelectClassTopNavigationWalker extends Walker_Nav_Menu {
		...
		function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			if($item->hide == ""){
				if($item->nolink == ""){
				    ...
				    $pos = strpos($attributes, ".pdf");
                    if($pos===false){
                        $item_output .= '<a'. $attributes .'><span class="item_outer">';
                    }else{
                        $item_output .= '<a'. $attributes .' download><span class="item_outer">';
                    }
