...
class AarhusCoreImport {
	...
	public function import_content( $file ) {
		ob_start();
		require_once( AARHUS_CORE_ABS_PATH . '/import/class.wordpress-importer.php' );
		$aarhus_import = new WP_Import();
		set_time_limit( 0 );
		
		$aarhus_import->fetch_attachments = $this->attachments;
		$returned_value                  = $aarhus_import->import( $file );
		
		if ( is_wp_error( $returned_value ) ) {
			$this->message = esc_html__( 'An Error Occurred During Import', 'aarhus-core' );
		} else {
			$this->message = esc_html__( 'Content imported successfully', 'aarhus-core' );
		}
	...
	public function import_widgets( $file, $file2 ) {
		...
		foreach ( (array) $options['widgets'] as $aarhus_widget_id => $aarhus_widget_data ) {
			update_option( 'widget_' . $aarhus_widget_id, $aarhus_widget_data );
		}
		
		$this->import_sidebars_widgets( $file );
		$this->message = esc_html__( 'Widgets imported successfully', 'aarhus-core' );
	}
	
	public function import_sidebars_widgets( $file ) {
		$aarhus_sidebars = get_option( "sidebars_widgets" );
		unset( $aarhus_sidebars['array_version'] );
		$data = $this->file_options( $file );
		
		if ( is_array( $data['sidebars'] ) ) {
			$aarhus_sidebars = array_merge( (array) $aarhus_sidebars, (array) $data['sidebars'] );
			unset( $aarhus_sidebars['wp_inactive_widgets'] );
			$aarhus_sidebars                  = array_merge( array( 'wp_inactive_widgets' => array() ), $aarhus_sidebars );
			$aarhus_sidebars['array_version'] = 2;
			wp_set_sidebars_widgets( $aarhus_sidebars );
		}
	}
	...
	public function import_custom_sidebars( $file ) {
		...
		$this->message = esc_html__( 'Custom sidebars imported successfully', 'aarhus-core' );
	}
	
	public function import_options( $file ) {
		$options       = $this->file_options( $file );
		$result        = update_option( 'qodef_options_aarhus', $options );
		$this->message = esc_html__( 'Options imported successfully', 'aarhus-core' );
	}
	
	public function import_menus( $file ) {
		global $wpdb;
		$aarhus_terms_table = $wpdb->prefix . "terms";
		$this->menus_data  = $this->file_options( $file );
		$menu_array        = array();
		
		foreach ( $this->menus_data as $registered_menu => $menu_slug ) {
			$term_rows = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM $aarhus_terms_table where slug=%s", $menu_slug ), ARRAY_A );
	...
	public function import_settings_pages( $file ) {
		...
		foreach ( $pages as $aarhus_page_option => $aarhus_page_id ) {
			update_option( $aarhus_page_option, $aarhus_page_id );
		}
	}
	...
	public function rev_sliders() {
		$rev_sldiers = array(
			...
			'landing-bottom.zip',
			'landing-portfolio-single.zip',
			'landing-top.zip',
			'main-home.zip',
			'one-page-home.zip',
			'shop-home.zip',
			'real-estate-home.zip',
		);
	...
	function qodef_admin_import() {
		if ( aarhus_core_theme_installed() ) {
			...
			$this->pagehook = add_submenu_page(
                SELECT_OPTIONS_SLUG,
				esc_html__( 'Aarhus Options - Aarhus Import', 'aarhus-core' ), // The value used to populate the browser's title bar when the menu page is active
				esc_html__( 'Import', 'aarhus-core' ),                     // The text of the menu in the administrator's sidebar
				'administrator',                                          // What roles are able to access the menu
                SELECT_OPTIONS_SLUG . $slug,                  // The ID used to bind submenu items to this menu
				array( $aarhus_select_global_Framework->getSkin(), 'renderImport' )
			);
			
			add_action( 'admin_print_scripts-' . $this->pagehook, 'aarhus_select_enqueue_admin_scripts' );
			add_action( 'admin_print_styles-' . $this->pagehook, 'aarhus_select_enqueue_admin_styles' );
		}
	}
	...
	function qodef_update_options_after_import( $folder ) {
		...
		foreach ( $demo_urls as $demo_url ) {
			$global_options    = get_option( 'qodef_options_aarhus' );
			$new_global_values = str_replace( $demo_url, $url, $global_options );

			update_option( 'qodef_options_aarhus', $new_global_values );
		}
	}
