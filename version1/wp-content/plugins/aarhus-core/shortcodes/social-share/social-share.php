<?php
namespace AarhusCore\CPT\Shortcodes\SocialShare;
...
class SocialShare implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if ( function_exists( 'vc_map' ) ) {
			vc_map(
				array(
					'name'                      => esc_html__( 'Social Share', 'aarhus-core' ),
					'base'                      => $this->getBase(),
					...
					'params'                    => array(
						array(
							...
							'value'      => array(
								esc_html__( 'List', 'aarhus-core' )     => 'list',
								esc_html__( 'Dropdown', 'aarhus-core' ) => 'dropdown',
								esc_html__( 'Text', 'aarhus-core' )     => 'text'
							)
						),
						array(
							'type'       => 'dropdown',
							'param_name' => 'dropdown_behavior',
							'heading'    => esc_html__( 'DropDown Hover Behavior', 'aarhus-core' ),
							'value'      => array(
								esc_html__( 'On Bottom Animation', 'aarhus-core' ) => 'bottom',
								esc_html__( 'On Right Animation', 'aarhus-core' )  => 'right',
								esc_html__( 'On Left Animation', 'aarhus-core' )   => 'left'
							),
						...
						array(
							...
							'heading'    => esc_html__( 'Icons Type', 'aarhus-core' ),
							'value'      => array(
								esc_html__( 'Font Awesome', 'aarhus-core' ) => 'font-awesome',
								esc_html__( 'Font Elegant', 'aarhus-core' ) => 'font-elegant'
							),
						...
						array(
							'type'       => 'textfield',
							'param_name' => 'title',
							'heading'    => esc_html__( 'Social Share Title', 'aarhus-core' )
						)
					)
				)
			);
	...
	public function render( $atts, $content = null ) {
		...
		$params['enable_social_share'] = aarhus_select_options()->getOptionValue( 'enable_social_share' ) === 'yes';
		
		//Is social share enabled for post type
		$post_type         = str_replace( '-', '_', get_post_type() );
		$params['enabled'] = aarhus_select_options()->getOptionValue( 'enable_social_share_on_' . $post_type ) === 'yes';
		...
		if ( $params['enable_social_share'] && $params['enabled'] ) {
			$html = aarhus_core_get_shortcode_module_template_part( 'templates/' . $params['type'], 'social-share', '', $params );
		}
	...
	private function getSocialNetworksParams( $params ) {
		...
		foreach ( $this->socialNetworks as $net ) {
			...
			if ( aarhus_select_options()->getOptionValue( 'enable_' . $net . '_share' ) == 'yes' ) {
				...
				$params['custom_icon'] = ( aarhus_select_options()->getOptionValue( $net . '_icon' ) ) ? aarhus_select_options()->getOptionValue( $net . '_icon' ) : '';
				
				$html = aarhus_core_get_shortcode_module_template_part( 'templates/parts/network', 'social-share', '', $params );
			}
    ...
    private function getSocialNetworkShareLink($net, $image) {
        switch ($net) {
            ...
            case 'twitter':
                ...
                $twitter_via = (aarhus_select_options()->getOptionValue('twitter_via') !== '') ? ' via ' . aarhus_select_options()->getOptionValue('twitter_via') . ' ' : '';
                if (wp_is_mobile()) {
                    $link = 'window.open(\'https://twitter.com/intent/tweet?text=' . urlencode(aarhus_select_the_excerpt_max_charlength($count_char) . $twitter_via) . get_permalink() . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');';
                } else {
                    $link = 'window.open(\'http://twitter.com/home?status=' . urlencode(aarhus_select_the_excerpt_max_charlength($count_char) . $twitter_via) . get_permalink() . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');';
                }
                break;
            case 'google_plus':
            ...
            case 'tumblr':
                ...
                break;
            case 'pinterest':
                $link = 'popUp=window.open(\'http://pinterest.com/pin/create/button/?url=' . urlencode(get_permalink()) . '&amp;description=' . aarhus_select_addslashes(get_the_title()) . '&amp;media=' . urlencode($image[0]) . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');popUp.focus();return false;';
                break;
            case 'vk':
	...
	private function getSocialNetworkText( $net ) {
		switch ( $net ) {
			case 'facebook':
				$text = esc_html__( 'fb', 'aarhus-core');
				break;
			case 'twitter':
				$text = esc_html__( 'tw', 'aarhus-core');
				break;
			case 'google_plus':
				$text = esc_html__( 'g+', 'aarhus-core');
				break;
			case 'linkedin':
				$text = esc_html__( 'lnkd', 'aarhus-core');
				break;
			case 'tumblr':
				$text = esc_html__( 'tmb', 'aarhus-core');
				break;
			case 'pinterest':
				$text = esc_html__( 'pin', 'aarhus-core');
				break;
			case 'vk':
				$text = esc_html__( 'vk', 'aarhus-core');
				break;
			default:
				$text = '';
		}
