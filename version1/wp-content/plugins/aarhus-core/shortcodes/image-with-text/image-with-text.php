<?php
namespace AarhusCore\CPT\Shortcodes\ImageWithText;
...
class ImageWithText implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if ( function_exists( 'vc_map' ) ) {
			vc_map(
				array(
					'name'                      => esc_html__( 'Image With Text', 'aarhus-core' ),
					'base'                      => $this->getBase(),
					...
					'params'                    => array(
						...
						array(
							...
							'value'      => array(
								esc_html__( 'Text Below Image' )  => 'below-image',
								esc_html__( 'Text Inside Image' ) => 'inside-image'
							)
						),
						array(
							'type'        => 'dropdown',
						...
						array(
							...
							'heading'    => esc_html__( 'Pre Title', 'aarhus-core' )
						),
						array(
							'type'       => 'colorpicker',
							'param_name' => 'pre_title_color',
							'heading'    => esc_html__( 'Pre Title Color', 'aarhus-core' ),
							'dependency' => array( 'element' => 'pre_title', 'not_empty' => true )
						),
						array(
							'type'       => 'textfield',
							'param_name' => 'title',
						...
						array(
							'type'       => 'textarea',
							'param_name' => 'pre_text',
							'heading'    => esc_html__( 'Pre Text', 'aarhus-core' )
						),
						array(
							'type'       => 'colorpicker',
							'param_name' => 'pre_text_color',
							'heading'    => esc_html__( 'Pre Text Color', 'aarhus-core' ),
							'dependency' => array( 'element' => 'pre_text', 'not_empty' => true )
						),
						array(
							'type'       => 'textarea',
							'param_name' => 'text',
	...
	public function render( $atts, $content = null ) {
		...
		$params['pre_title_styles']   = $this->getPreTitleStyles( $params );
		$params['pre_text_styles']    = $this->getPreTextStyles( $params );
		$params['text_styles']        = $this->getTextStyles( $params );
		
		$html = aarhus_core_get_shortcode_module_template_part( 'templates/image-with-text', 'image-with-text', '', $params );
	...
	private function getHolderClasses( $params ) {
		...
		$holderClasses[] = ! empty( $params['type'] ) ? 'qodef-type-' . $params['type'] : '';
	...
	private function getPreTitleStyles( $params ) {
		$styles = array();

		if ( ! empty( $params['pre_title_color'] ) ) {
			$styles[] = 'color: ' . $params['pre_title_color'];
		}
	...
	private function getPreTextStyles( $params ) {
		$styles = array();

		if (! empty ( $params['pre_text_color'] ) ) {
			$styles[] = 'color: '.$params['pre_text_color'];
		}
