...
namespace AarhusCore\CPT\Shortcodes\Testimonials;
...
class Testimonials implements Lib\ShortcodeInterface {
					...
					'params'                    => array(
						array(
							...
							'value'       => array(
								...
								esc_html__( 'Boxed', 'aarhus-core' )               => 'boxed',
								esc_html__( 'Image Pagination', 'aarhus-core' )    => 'image-pagination',
								esc_html__( 'Boxed Text', 'aarhus-core' )          => 'boxed-text',
						...
						array(
							'type'       => 'textfield',
							'param_name' => 'number',
							'heading'    => esc_html__( 'Number of Testimonials', 'aarhus-core' )
						),
						array(
							'type'        => 'autocomplete',
							'param_name'  => 'category',
							'heading'     => esc_html__( 'Category', 'aarhus-core' ),
							'description' => esc_html__( 'Enter one category slug (leave empty for showing all categories)', 'aarhus-core' )
						),
						array(
							'type'       => 'colorpicker',
							'param_name' => 'box_color',
							'heading'    => esc_html__( 'Content Box Color', 'aarhus-core' ),
						...
						array(
							'type'        => 'dropdown',
							'param_name'  => 'slider_pagination_position',
							'heading'     => esc_html__( 'Slider Pagination Position', 'aarhus-core' ),
							'value'       => array(
	...
	public function render( $atts, $content = null ) {
		$args   = array(
			...
			'slider_pagination_position'  => 'bottom'
		);
        ...
        return aarhus_core_get_cpt_shortcode_module_template_part( 'testimonials', 'testimonials', 'testimonials-' . $params['type'], '', $params );
	...
	private function getHolderClasses( $params ) {
		...
		$holderClasses[] = 'qodef-testimonials-pagination-position-' . $params['slider_pagination_position'];
	...
	public function testimonialsCategoryAutocompleteSuggester( $query ) {
		...
		if ( is_array( $post_meta_infos ) && ! empty( $post_meta_infos ) ) {
			foreach ( $post_meta_infos as $value ) {
				...
				$data['label'] = ( ( strlen( $value['testimonials_category_title'] ) > 0 ) ? esc_html__( 'Category', 'aarhus-core' ) . ': ' . $value['testimonials_category_title'] : '' );
				$results[]     = $data;
			}
		}
		...
		if ( ! empty( $query ) ) {
			...
			if ( is_object( $testimonials_category ) ) {
				...
				if ( ! empty( $testimonials_category_title ) ) {
					$testimonials_category_title_display = esc_html__( 'Category', 'aarhus-core' ) . ': ' . $testimonials_category_title;
				}
